...
const getErrorText = (error) =>
  error?.message || error?.error || "Admin API request failed";
...
const normalizeBackendUsers = (backendUsers = [], fallbackUsers = []) => {
  ...
  return backendUsers.map((backendUser, index) => {
    const fallback = fallbackUsers.find(
      (item) => item.id === backendUser.id || item.email === backendUser.email,
    ) ||
      fallbackUsers[index % fallbackUsers.length] || {
        name: "User",
    ...
    return {
      ...
      targetRole:
        ...
        backendUser.profile?.targetRole ||
        fallback.targetRole,
      location:
        backendUser.location ||
        backendUser.profile?.location ||
        fallback.location,
      lastActive: formatDateTime(
        backendUser.updatedAt || backendUser.lastLoginAt,
      ),
...
function AdminDashboard() {
  ...
  const [isAuthed, setIsAuthed] = useState(
    () => sessionStorage.getItem("hirepilot-admin") === "true",
  );
  ...
  const refreshAdminApiData = async () => {
    ...
    try {
      ...
      const nextUsers = normalizeBackendUsers(
        bundle.users,
        getStoredAdminUsers(),
      );
  ...
  const dsaScore = useMemo(
    () => calculateUserDsaScore(selectedUser?.dsa),
    [selectedUser],
  );
  ...
  const backendUsageData = useMemo(() => {
    const usage =
      backendBundle?.analytics?.algorithmAnalysis?.aiUsageByFeature || {};
  ...
  const backendOverview = useMemo(() => {
    ...
    return [
      ...
      [
        "Interviews",
        analytics.interviews ?? backendBundle?.interviews?.length ?? 0,
      ],
  ...
  const handleLogin = (event) => {
    ...
    setError(
      "Invalid admin credentials. Check VITE_ADMIN_EMAIL and VITE_ADMIN_PASSWORD in client/.env.",
    );
  };
  ...
  const handleDeleteSelectedUser = async () => {
    ...
    const confirmed = window.confirm(
      `Delete ${selectedUser.name}? This calls the backend admin delete route.`,
    );
    if (!confirmed) return;
        ...
        <div className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-6xl items-center justify-center">
          <div className="grid w-full gap-5 lg:grid-cols-[0.9fr_1.1fr]">
            <div className="dark-card rounded-2xl p-7 sm:p-10">
              <button
                onClick={() => navigate("/")}
                className="btn-secondary mb-8 h-11 w-11 min-h-0 rounded-full p-0"
              >
              ...
              <h1 className="mt-6 text-3xl font-black leading-tight text-white sm:text-5xl">
                HirePilot AI Admin Console
              </h1>
              <p className="mt-4 max-w-xl text-sm leading-relaxed text-slate-300 sm:text-base">
                Secure admin access for user data, suite scores, realtime
                dashboard updates, DSA calibration, and platform analytics.
              </p>
            </div>

            <form
              onSubmit={handleLogin}
              className="glass-panel rounded-2xl p-7 sm:p-10"
            >
              ...
              {error && (
                <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm font-semibold text-red-700 ring-1 ring-red-100">
                  {error}
                </p>
              )}
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-3 max-w-3xl text-slate-500">
                  Select a user to open their complete analytics, scores,
                  website activity, DSA data, and suite publishing controls.
                </p>
              </div>
            </div>
            <div className="flex flex-col gap-3 sm:flex-row">
              <button
                onClick={refreshAdminApiData}
                disabled={isApiLoading}
                className="btn-secondary w-fit px-5 py-2"
              >
                <FaDatabase />{" "}
                {isApiLoading ? "Loading API" : "Refresh backend"}
              </button>
              <button
              ...
              <button
                ...
                className="btn-danger w-fit px-5 py-2"
              >
                <FaSignOutAlt /> Logout
              </button>
            </div>
            ...
            <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  The first admin view is intentionally limited to the user
                  list.
                </p>
                <p
                  className={`mt-2 text-xs font-bold ${apiError ? "text-amber-700" : "text-emerald-700"}`}
                >
                  {apiError
                    ? `Backend API fallback: ${apiError}`
                    : apiSyncedAt
                      ? `Backend synced: ${apiSyncedAt}`
              ...
              <div className="divide-y divide-slate-100">
                {filteredUsers.map((user) => (
                  <button
                    ...
                    className="grid w-full gap-4 px-5 py-5 text-left transition hover:bg-emerald-50/60 lg:grid-cols-[1.2fr_0.8fr_0.7fr_0.7fr_0.6fr] lg:items-center"
                  >
                    ...
                    <span className="w-fit rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-700 ring-1 ring-slate-200">
                      {user.plan}
                    </span>
                    <span>
                      ...
                      <span
                        className={`mt-1 inline-flex rounded-full px-2.5 py-1 text-xs font-black ring-1 ${getRiskClass(user.risk)}`}
                      >
                        {user.risk}
                      </span>
                    </span>
      ...
      <div className="wide-shell">
        <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="eyebrow mb-3">
                <FaShieldAlt /> User Analytics
              </p>
              <h1 className="text-3xl font-black text-slate-950 sm:text-5xl">
                {selectedUser.name}
              </h1>
              <p className="mt-3 max-w-3xl text-slate-500">
                {selectedUser.email} - {selectedUser.targetRole} -{" "}
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <button
              onClick={publishUserToSuiteDashboard}
        ...
        <section className="grid gap-4 md:grid-cols-4">
          {[
            ...
            [
              "AI credits",
              `${selectedUser.credits.remaining}/${selectedUser.credits.limit}`,
              FaBroadcastTower,
            ],
            ...
            <div
              key={label}
              className="rounded-xl border border-slate-800 bg-slate-950 p-5 text-white"
            >
        ...
        <section className="mt-5 grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
          <div className="surface-card p-5 sm:p-6">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Live values from admin analytics, interviews, and reports routes
              when the backend ADMIN session is available.
            </p>
            ...
            <button
              onClick={refreshAdminApiData}
              disabled={isApiLoading}
              className="btn-secondary mt-5 w-full px-5 py-3"
            >
              <FaDatabase />{" "}
              {isApiLoading
                ? "Refreshing backend"
          ...
          <div className="surface-card p-5 sm:p-6">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Bar graph from /api/admin/analytics
              algorithmAnalysis.aiUsageByFeature.
            </p>
            <div className="mt-5 h-72">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={
                    backendUsageData.length
                      ? backendUsageData
                      : [{ feature: "No API data", requests: 0, credits: 0 }]
                  }
                >
          ...
          <div className="surface-card p-5 sm:p-6">
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  User activity, interview volume, and AI usage over recent
                  sessions.
                </p>
              </div>
          ...
          <div className="surface-card p-5 sm:p-6">
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Plan consumption for this user.
                </p>
              </div>
        ...
        <section className="mt-5 grid gap-5 xl:grid-cols-[1fr_1fr]">
          <div className="surface-card p-5 sm:p-6">
            <h2 className="text-2xl font-black text-slate-950">
              Suite Score Controls
            </h2>
            <p className="mt-1 text-sm text-slate-500">
              Edit per-user score values, then publish them to the main suite
              dashboard.
            </p>
              ...
              {userScoreFields.map((field) => (
                ...
                >
                  ...
                  <input
                    ...
                    value={selectedUser.scores[field.key] ?? 0}
                    onChange={(event) =>
                      updateUserScore(field.key, event.target.value)
                    }
                    className="input-field p-3"
                  />
                </label>
          ...
          <div className="surface-card p-5 sm:p-6">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Bar graph generated from the selected user's suite data.
            </p>
        ...
        <section className="mt-5 grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
          <div className="surface-card p-5 sm:p-6">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Flowchart built from this user's job pipeline.
            </p>
            <div className="mt-6 flex flex-col gap-3">
              {selectedUser.funnel.map((step, index) => (
                <div key={step.label} className="flex items-center gap-3">
                  ...
                  {index < selectedUser.funnel.length - 1 && (
                    <FaChevronRight className="hidden shrink-0 text-emerald-700 sm:block" />
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="surface-card p-5 sm:p-6">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Solved rate, review quality, streak, and weak-topic penalty update
              this user's DSA score.
            </p>
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              {[
                ...
                <label key={key}>
                  ...
                  <input
                    type="number"
                    value={selectedUser.dsa[key]}
                    onChange={(event) =>
                      updateDsaField(key, event.target.value)
                    }
                    className="input-field p-3"
                  />
                </label>
        ...
        <section className="mt-5 grid gap-5 xl:grid-cols-[1fr_1fr]">
          <div className="surface-card p-5 sm:p-6">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Portfolio and website signals connected to this user.
            </p>
          ...
          <div className="surface-card p-5 sm:p-6">
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-black text-slate-950">
                  Realtime Suite Notice
                </h2>
                <p className="mt-1 text-sm text-slate-500">
                  Publish live admin data visible in the suite dashboard.
                </p>
              </div>
            ...
            <div className="space-y-4">
              ...
              <label>
                ...
                <textarea
                  ...
                  className="input-field min-h-24 p-3"
                />
              </label>
              <label>
                <span className="mb-2 block text-sm font-bold text-slate-700">
          ...
          {dashboardStatus.map((item) => (
            <div key={item.label} className="surface-card p-5">
              ...
              <div className="mt-4 space-y-3">
                <input
                  value={statusDraft[item.label]?.value || ""}
                  onChange={(event) =>
                    updateStatusField(item.label, "value", event.target.value)
                  }
                  className="input-field p-3"
                />
                <textarea
                  value={statusDraft[item.label]?.detail || ""}
                  onChange={(event) =>
                    updateStatusField(item.label, "detail", event.target.value)
                  }
                  className="input-field min-h-20 p-3"
                />
                <button
                  onClick={() =>
                    updateDashboardStatus(item.label, statusDraft[item.label])
                  }
        ...
        <p className="mt-5 text-xs font-semibold text-slate-400">
          Last suite state update:{" "}
          {dashboardState.updatedAt
            ? new Date(dashboardState.updatedAt).toLocaleString()
            : "Not published yet"}
        </p>
      </div>
    </div>
